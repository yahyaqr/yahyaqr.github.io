...
export function normalizeSubdomainRecord(record) {
  ...
  return {
    id: subdomain,
    subdomain,
    type: record?.type === 'redirect' ? 'redirect' : 'page',
    title: String(record?.title || '').trim(),
    content: record?.type === 'redirect' ? null : record?.content || {},
    targetUrl: record?.type === 'redirect' ? String(record?.targetUrl || '').trim() : String(record?.targetUrl || '').trim(),
    theme: {
...
export function validateSubdomainRecord(record, records = []) {
  ...
  if (!['page', 'redirect'].includes(normalized.type)) {
    errors.push('Type must be either "page" or "redirect".');
  }
